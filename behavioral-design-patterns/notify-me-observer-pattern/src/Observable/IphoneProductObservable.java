...
public class IphoneProductObservable
        implements StockAvailabilityObservable {
    ...
    @Override
    public void addStockObserver(
            StockNotificationObserver observer
    ) {
    ...
    @Override
    public void removeStockObserver(
            StockNotificationObserver observer
    ) {
        ...
        System.out.println(
                "[-] " + observer.getUserId() +
                        " unsubscribed for notifications on " +
                        productName
        );
    }
    ...
    public void notifyStockObservers() {
        ...
        if (stockQuantity > 0 && !stockObservers.isEmpty()) {
            ...
            List<StockNotificationObserver> observersToNotify =
                    new ArrayList<>(stockObservers);

            for (StockNotificationObserver observer :
                    observersToNotify) {
    ...
    public void restock(int quantity) {
        ...
        // Notify only if product was previously out of stock
